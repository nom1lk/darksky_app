...
# rails new darksky2 --database=postgresql
# rails g model Condition
...
# rake db:create
# rake db:migrate
# if anything didn't work, try rake db:drop after creating the app (may have been another app/model of the same name(s) which needs to be deleted, not just by rm -r on the app, but also by deleting the postgresql tables)
